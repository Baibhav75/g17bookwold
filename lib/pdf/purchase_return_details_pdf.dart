...
import '../Model/purchase_return_details_model.dart'; // Make sure this path is correct

class PurchaseReturnDetailsPdf {
  static Future<File> generate(PurchaseReturnDetailsModel data) async {
    ...
    final file = File("${dir.path}/purchase_return_details_${data.billNo}.pdf");
  ...
  static pw.Widget _buildInfoTable(PurchaseReturnDetailsModel data, String formattedDate) {
    return pw.Table(
      ...
      children: [
        ...
        pw.TableRow(
          children: [
            _infoCell("Supplier Invoice No: ", "1"), // Fallback as requested
            _infoCell("Address: ", ""), // Fallback 
            _infoCell("Rec. Date: ", formattedDate),
          ],
        ),
        pw.TableRow(
          children: [
            _infoCell("Transport: ", "SELF"), // Fallback
            _infoCell("GR No: ", "1"), // Fallback
            pw.SizedBox(),
          ],
        ),
      ],
  ...
  static pw.Widget _buildItemsTable(PurchaseReturnDetailsModel data) {
    ...
    // As the JSON model doesn't contain discount, default discount amount to 0
    ...
    grouped.forEach((series, items) {
      ...
      for (var e in items) {
        ...
        // No discount available in model so itemDiscAmount is 0
        double itemDiscAmount = 0;
        double itemAmountWithDiscount = e.totalAmount;
    ...
    rows.add(
      pw.TableRow(
        ...
        children: [
          ...
          pw.SizedBox(),
          pw.SizedBox(),
          _cell("₹ ${totalAmountWithDiscount.toStringAsFixed(2)}",
              align: pw.TextAlign.center, isBold: true),
        ],
      ),
    );
